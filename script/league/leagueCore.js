...
import { updateMonthUI } from "./leagueEvents.js";
...
export async function autoInitLeague() {
  ...
  // Filtro global de gênero para não misturar competições
  const femKeywords = [
    "Vrouwen",
    "Fem",
    "NWSL",
  ...
  let filteredTeams = availableTeams.filter((t) => {
    ...
    return isFemale ? isFemTeam : !isFemTeam;
  });
  ...
  const isSouthAm = southAmKeywords.some((k) =>
    leagueName.toLowerCase().includes(k),
  );
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const groupMatches = [
      {
        home: groupTeams[0].id,
        away: groupTeams[1].id,
        played: false,
        homeScore: null,
        awayScore: null,
        round: 1,
      ...
      {
        home: groupTeams[2].id,
        away: groupTeams[3].id,
        played: false,
        homeScore: null,
        awayScore: null,
        round: 1,
  ...
  console.log(
    `[League] Auto-inicializada: ${leagueName}, ${cupData.phaseNames[0]} e ${contName}.`,
  );
}

export async function createNewLeague(
  leagueType = "br",
  existingDivisions = null,
  gender = "male",
  qualifiedIds = [],
) {
  ...
  const isEuro =
    leagueType !== "br" &&
    euroKeywords.some((k) => leagueType.toLowerCase().includes(k));
    ...
    let leaguePool = pool.filter((t) => {
      ...
      if (
        ...
        (ln.includes("brazil") ||
          ...
          ln.includes("primera") ||
          ln.includes("colombia") ||
          ln.includes("argentina") ||
          ln.includes("serie b") ||
          ln.includes("br"))
      )
        return true;
      if (
        ...
        (ln.includes("premier") ||
          ln.includes("england") ||
          ln.includes("championship") ||
          ln.includes("league one") ||
          ln.includes("league two") ||
          ln.includes("wsl"))
      )
        return true;
      if (
        leagueType === "es" &&
        (ln.includes("liga") || ln.includes("spain") || ln.includes("segunda"))
      )
        return true;
      if (
        leagueType === "it" &&
  ...
  let continentalName =
    leagueType === "br" ? "Copa Libertadores" : "Champions League";
  let contTeamsInfo = [];
  ...
  if (qualifiedIds && qualifiedIds.length > 0) {
    ...
    qualifiedIds.forEach((id) => {
      ...
      if (found)
        contTeamsInfo.push({ id: found.id, name: found.name, ovr: found.ovr });
    });
  } else {
  ...
  // Filtro global de gênero para os estrangeiros (Temporadas seguintes)
  const femKeywordsForeign = [
    "Vrouwen",
    "Fem",
    "NWSL",
    "WSL",
  ...
  availableTeams = availableTeams.filter((t) => {
    const isFemTeam =
      t.name.includes("(Fem)") ||
      femKeywordsForeign.some((k) => (t.league || "").includes(k));
    return gender === "female" ? isFemTeam : !isFemTeam;
  });

  // Filtramos times estrangeiros para completar os 32
  let foreignTeams = availableTeams.filter(
    (t) => !divisions.some((d) => d.table.some((dt) => dt.id === t.file)),
  );
  ...
  let initialDivision = divisions.findIndex((d) =>
    d.table.some((t) => t.isUser),
  );
...
export async function simulateCurrentRound(silent = false) {
    ...
    if (nextCupDate <= currentLeagueDate) {
      ...
      matches.forEach((m) => {
        if (!m.played) {
          ...
          if (isFinal) {
            ...
            const idaMatch = data.cup.phases[phaseIdx - 1].find(
      ...
      if (isIda) {
        ...
        matches.forEach((m) => {
          ...
          if (
            aggAway > aggHome ||
            (aggAway === aggHome && m.homePen > m.awayPen)
          )
            winners.push(m.home);
  ...
  if (data.continentalCup && !data.continentalCup.finished) {
    const totalRounds = Math.max(...data.divisions.map((d) => d.rounds.length));
    const nextContDate = getMatchDate(
      "continental",
      ...
      if (phaseIdx < 3) {
        ...
        data.continentalCup.groups.forEach((group) => {
          ...
          roundMatches.forEach((m) => {
            if (!m.played) {
              ...
              m.awayScore = aScore;
              m.played = true;
              const hTeam = group.teams.find((t) => t.id === m.home),
                aTeam = group.teams.find((t) => t.id === m.away);
              hTeam.p++;
              aTeam.p++;
              hTeam.gf += hScore;
              hTeam.ga += aScore;
        ...
        matches.forEach((m) => {
          if (!m.played) {
            ...
            if (isFinal) {
              ...
              const idaMatch = data.continentalCup.phases[knockoutIdx - 1].find(
                (im) => im.home === m.away && im.away === m.home,
              );
              if (
                idaMatch &&
                idaMatch.homeScore + aScore === idaMatch.awayScore + hScore
              ) {
                hPen = Math.floor(Math.random() * 4) + 2;
                aPen = Math.floor(Math.random() * 4) + 2;
                if (hPen === aPen) hPen++;
        ...
        if (isIda) {
          ...
          matches.forEach((m) => {
            ...
            if (
              idaMatch.awayScore + m.homeScore >
                idaMatch.homeScore + m.awayScore ||
              (idaMatch.awayScore + m.homeScore ===
                idaMatch.homeScore + m.awayScore &&
                m.homePen > m.awayPen)
            )
  ...
  data.divisions.forEach((div) => {
    if (data.currentRound <= div.rounds.length) {
      const match = div.rounds[data.currentRound - 1].find(
        (m) =>
          div.table.find((t) => t.id === m.home)?.isUser ||
          div.table.find((t) => t.id === m.away)?.isUser,
      );
