...
import { isTableView, renderTable } from "./uiTableView.js";
...
// Função tradutora para limpar mensagens velhas corrompidas salvas no Banco de Dados
const fixEncoding = (str) => {
  if (typeof str !== "string") return str;
  try {
    if (str.includes("Ã") || str.includes("â") || str.includes("ð")) {
...
export function renderTeamStats() {
  ...
  const topScorers = [...squad].sort((a, b) => (b.goals || 0) - (a.goals || 0));
...
export async function getNewsList(info, includeArchived = false) {
  ...
  if (history && history.length > 0) {
    ...
    lastMatches.forEach((m, i) => {
      ...
      news.push({
        ...
        desc: `Pós-jogo: ${m.home || "Seu Time"} vs ${m.away || "Adversário"} (${compName}).`,
        longDesc: `Prezado Treinador,<br><br>A partida válida pela <strong>${compName}</strong> entre <strong>${m.home || "Seu Time"}</strong> e <strong>${m.away || "Adversário"}</strong> terminou em <strong>${scoreDisplay}</strong>.<br><br>Nossa análise indica que a equipe se comportou de forma ${m.result === "V" ? "excelente, com grande entrega coletiva" : m.result === "D" ? "abaixo do esperado, precisamos revisar os pontos táticos" : "regular, com equilíbrio entre os times"}. O placar de <strong>${scoreDisplay}</strong> reflete o que vimos em campo.<br><br>Seguimos focados na preparação para o próximo compromisso.`,
  ...
  const fifaEvent = getFIFAEvent(
    info.currentDate,
    info.calendarType || "europe",
  );
  if (fifaEvent) {
    ...
    news.push({
      ...
      desc: isTournament
        ? "O maior torneio de seleções do planeta!"
        : "O calendário de clubes está pausado para as datas internacionais.",
      longDesc: `Prezado Treinador,<br><br>O calendário oficial de clubes está temporariamente suspenso para a realização do período de seleções nacionais.<br><br><strong>Evento:</strong> ${fifaEvent.name}<br><strong>Status:</strong> Ativo<br><br>${isTournament ? "As maiores seleções do mundo disputam a fase final da Copa Internacional. São 16 seleções disputando o título máximo em jogos mata-mata eletrizantes!" : "Seus atletas de elite viajaram para representar suas respectivas seleções. Esta semana é uma excelente oportunidade para descansar e regenerar o elenco."}<br><br>Clique abaixo para simular as rodadas internacionais.`,
      actionLabel: isTournament
        ? "SIMULAR COPA INTERNACIONAL"
        : "SIMULAR DATA FIFA",
      actionView: "dashboard",
    });
  } else if (leagueData && !leagueData.finished) {
    const round =
      leagueData.divisions?.[0]?.rounds?.[leagueData.currentRound - 1];
    const userMatch = round?.find(
      (m) => m.home === info.teamFile || m.away === info.teamFile,
    );
    if (userMatch) {
      const opponent =
      ...
      news.push({
        ...
        longDesc: `O clima no vestiário é de foco total para o próximo confronto contra o <strong>${opponent}</strong>, válido pela <strong>${leagueName}</strong> — Rodada ${leagueData.currentRound}.<br><br>Jogaremos <strong>${stadium}</strong>. Nossa equipe de análise já mapeou os pontos fortes e fracos do adversário. É essencial que a prancheta tática esteja definida e que os jogadores estejam em plenas condições físicas.<br><br>A torcida já está mobilizada para este importante duelo!`,
  ...
  const sortedByRating = [...squad]
    .filter((p) => p.ratingHistory && p.ratingHistory.length > 0)
    .sort((a, b) => {
      const avgA =
        a.ratingHistory.reduce((s, v) => s + v, 0) / a.ratingHistory.length;
      const avgB =
        b.ratingHistory.reduce((s, v) => s + v, 0) / b.ratingHistory.length;
      return avgB - avgA;
    });

  if (sortedByRating.length > 0) {
    const p = sortedByRating[0];
    const notes = p.ratingHistory
      .map(
        (r) =>
          `<span style="background: ${r >= 7 ? "var(--rating-high)" : r >= 6 ? "var(--warning)" : "#ff4444"}; color: #000; padding: 2px 6px; border-radius: 4px; font-weight: bold; margin-right: 5px;">${r.toFixed(1)}</span>`,
      )
      .join(" ");
    news.push({
      id: `performance-top`,
      ...
      desc: `Melhor média recente: ${(p.ratingHistory.reduce((s, v) => s + v, 0) / p.ratingHistory.length).toFixed(1)} — notas: ${p.ratingHistory.join(" | ")}`,
      longDesc: `Temos um relatório sobre o desempenho recente de <strong>${p.name}</strong>.<br><br>Nas últimas partidas, o atleta manteve uma regularidade impressionante. Confira as notas das últimas atuações:<br><br>${notes}<br><br>Este nível de consistência é fundamental para a estabilidade tática da equipe.`,
  ...
  const lowFitness = squad.filter(
    (p) => p.fitness < 60 && p.matchStatus === "normal",
  );
  if (lowFitness.length > 0) {
    news.push({
      id: `fitness-alert-${lowFitness
        .map((p) => p.id)
        .sort()
        .join("-")}`,
      type: "fitness",
      icon: "alert-triangle",
      color: "#ff4444",
      title: "Alerta de Desgaste Físico",
      desc: `${lowFitness[0].name}${lowFitness.length > 1 ? ` e mais ${lowFitness.length - 1}` : ""} com energia crítica.`,
      longDesc: `O departamento médico emitiu um alerta sobre a condição física dos seguintes atletas:<br><br><strong>${lowFitness.map((p) => `${p.name} (${Math.floor(p.fitness)}%)`).join("<br>")}</strong><br><br>Esses jogadores apresentam níveis críticos de fadiga e correm risco de lesão caso sejam escalados sem o devido descanso. Sugerimos um rodízio no elenco ou carga reduzida nos treinos.`,
  ...
  if (star && (star.rating || 0) > 85) {
    news.push({
      ...
      desc: `${star.name} está em excelente forma técnica (OVR ${Math.round(star.rating || star.ovr)}).`,
      longDesc: `Temos o prazer de informar que <strong>${star.name}</strong> tem se destacado nos treinamentos desta semana de forma excepcional.<br><br>Com overall de <strong>${Math.round(star.rating || star.ovr)}</strong>, o atleta é atualmente o melhor classificado do seu elenco. A dedicação e o desempenho técnico do jogador têm elevado o nível de competitividade do grupo e servindo de inspiração para os mais jovens.`,
      actionLabel: "GERENCIAR ELENCO",
      actionView: "table",
    });
  }
  ...
  if (isWindow) {
    news.push({
      ...
      desc: "A janela está aberta. Este é o momento de agir no mercado.",
      longDesc: `A janela de transferências foi oficialmente aberta para este período da temporada!<br><br>Este é o momento ideal para reforçar o elenco ou negociar atletas que não fazem parte dos planos futuros da comissão técnica. O mercado está aquecido e diversas oportunidades de negócio podem surgir a qualquer momento.<br><br>Fique atento às movimentações dos outros clubes e aos valores de mercado.`,
...
async function renderNewsFeed(info) {
  ...
  news.forEach((item) => {
    ...
    const desc = fixEncoding(item.desc);
    ...
    msg.innerHTML = `
      ...
      <div style="flex: 1;">
        <div style="font-weight: bold; color: #fff; font-size: 0.85rem; margin-bottom: 3px;">${title}</div>
        <div style="font-size: 0.75rem; color: #999; line-height: 1.4;">${desc}</div>
      </div>
    `;
...
export async function renderInbox(selectedId = null, showArchived = false) {
  ...
  // Lê o inbox persistido (histórico real salvo)
  const persistedInbox = await Storage.getInbox();

  // Lê também as mensagens dinâmicas atuais (propostas ativas) que não são persistidas
  const allDynamic = await getNewsList(coach, true);
  const dynamicProposals = allDynamic.filter((n) => n.type === "proposal");

  // Merge: propostas dinâmicas + histórico persistido (sem duplicar por id)
  ...
  if (!toggleContainer) {
    ...
    toggleContainer.style.cssText =
      "padding: 10px 15px; border-bottom: 1px solid #222; display: flex; justify-content: space-between; align-items: center; background: #111;";
  ...
  toggleContainer.innerHTML = `
    ...
    <div style="display: flex; gap: 8px;">
      ${!showArchived && news.length > 0 ? `<button id="btnMarkAllRead" style="background: transparent; border: 1px solid #333; color: #888; font-size: 0.6rem; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-weight: bold; transition: all 0.2s;" onmouseover="this.style.color='#fff'; this.style.borderColor='#555'" onmouseout="this.style.color='#888'; this.style.borderColor='#333'">MARCAR TODAS COMO LIDAS</button>` : ""}
  ...
  `;
  document.getElementById("btnToggleArchive").onclick = () =>
  ...
  if (btnMarkAll) {
    btnMarkAll.onclick = async () => {
      ...
      if (!coachInfo.archivedNews) coachInfo.archivedNews = [];
      news.forEach((n) => {
        if (!coachInfo.archivedNews.includes(n.id)) {
          coachInfo.archivedNews.push(n.id);
        }
      });
  ...
  news.forEach((item) => {
    ...
    msg.innerHTML = `
      <div style="font-weight: bold; color: ${isSelected ? "var(--accent)" : "#fff"}; font-size: 0.8rem; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${title}</div>
      <div style="font-size: 0.7rem; color: #666; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${desc}</div>
      ${dateLabel}
    `;
  ...
  content.innerHTML = `
    <div style="padding: 30px; display: flex; flex-direction: column; height: 100%;">
      ...
      <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #222; display: flex; gap: 10px; justify-content: flex-end;">
        ...
        <button id="inboxDownloadBtn" style="width: 140px; height: 45px; background: transparent; border: 1px solid #333; color: #aaa; border-radius: 8px; font-weight: bold; cursor: pointer; font-size: 0.8rem; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 6px;" onmouseover="this.style.color='#fff'; this.style.borderColor='#555'" onmouseout="this.style.color='#aaa'; this.style.borderColor='#333'">
          <i data-lucide="download" style="width: 1rem; height: 1rem;"></i> SALVAR .TXT
        </button>
  ...
  const downloadBtn = document.getElementById("inboxDownloadBtn");
  if (downloadBtn) {
    downloadBtn.onclick = () => {
      const tempDiv = document.createElement("div");
      tempDiv.innerHTML = selectedItem.longDesc || selectedItem.desc;
      const textContent = tempDiv.innerText || tempDiv.textContent;
      const dateStr =
        selectedItem.savedDate ||
        (selectedItem.timestamp
          ? new Date(selectedItem.timestamp).toLocaleDateString("pt-BR")
          : "Hoje");
      const fileContent = `DE: Diretoria Executiva\nDATA: ${dateStr}\nASSUNTO: ${selectedItem.title}\n\n${textContent}\n\nAtenciosamente,\nDiretoria Executiva - FutDashboard FC`;
      ...
      a.href = url;
      a.download = `Email_${selectedItem.title.replace(/[^a-z0-9]/gi, "_").toLowerCase()}.txt`;
      a.click();
  ...
  if (archiveBtn) {
    archiveBtn.onclick = async () => {
      ...
      if (showArchived) {
        coach2.archivedNews = coach2.archivedNews.filter(
...
async function renderCalendar(info) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const cellFifaEvent = getFIFAEvent(
      cellDateStr,
      info.calendarType || "europe",
    );
...
export async function renderProposals(
  info,
  containerId = "proposalsList",
  badgeId = "proposalCountBadge",
  cardId = "transferProposalsCard",
) {
  ...
  if (badge)
    badge.innerText = `${proposals.length} Pendente${proposals.length > 1 ? "s" : ""}`;
  list.innerHTML = "";

  proposals.forEach((prop) => {
    const item = document.createElement("div");
    item.style.cssText =
      "background: #1a1a1a; border: 1px solid #333; padding: 15px; border-radius: 10px; display: flex; flex-direction: column; gap: 10px;";
  ...
  list.querySelectorAll(".accept-prop-btn").forEach((btn) => {
    btn.onclick = async () => {
      ...
      const confirm = await showCustomModal(
        `Deseja aceitar a proposta de <b>${formatMoney(prop.value)}</b> de <b>${prop.from}</b> por <b>${prop.playerName}</b>?`,
        "confirm",
        "btn-primary",
      );
      if (confirm) {
...
export async function renderPlayerDetail(playerId) {
  ...
  // Eventos de Gestão
  const { sellPlayer, releasePlayer } =
    await import("../player/playerEditor.js");

  document.getElementById("detailSellBtn").onclick = () => sellPlayer(p.id);
  document.getElementById("detailReleaseBtn").onclick = () =>
    releasePlayer(p.id);
...
export async function renderNegotiation(
  playerId,
  propId = null,
  isBuying = false,
  externalPlayerData = null,
) {
  ...
  if (acceptBtn) {
    acceptBtn.onclick = async () => {
      const confirm = await showCustomModal(
        `Confirmar venda de <strong>${player.name}</strong> para o <strong>${proposal.from}</strong> por <strong>${formatMoney(proposal.value)}</strong>?`,
        "confirm",
        "btn-primary",
      );
      if (confirm) {
        ...
        showCustomModal(
          `Venda concluída! O saldo do clube agora é ${formatMoney(coach.budget)}`,
          "alert",
          "btn-primary",
        );
  ...
  if (rejectBtn) {
    rejectBtn.onclick = async () => {
      ...
      await Storage.saveCoachInfo(coach);
      showCustomModal(
        `Proposta recusada. Os negociadores do <strong>${proposal.from}</strong> deixaram a mesa.`,
        "alert",
        "btn-secondary",
      );
  ...
  if (submitBtn) {
    submitBtn.onclick = async () => {
      ...
      if (offerValue > coach.budget) {
        showCustomModal(
          "Saldo insuficiente para realizar esta oferta.",
          "alert",
          "btn-danger",
        );
        return;
      ...
      const confirm = await showCustomModal(
        `Deseja enviar oferta de <strong>${formatMoney(offerValue)}</strong> para contratar <strong>${player.name}</strong>?`,
        "confirm",
        "btn-primary",
      );
      if (confirm) {
        const successChance =
          offerValue >= player.marketValue
            ? 0.9
            : (offerValue / player.marketValue) * 0.8;
        if (Math.random() < successChance) {
          coach.budget -= offerValue;
          if (!coach.transactions) coach.transactions = [];
          coach.transactions.push({
            type: "expense",
            desc: `Contratação de ${player.name} (${player.clubName || "Livre"})`,
            amount: offerValue,
            date: new Date().toLocaleDateString("pt-BR"),
          });
          await Storage.saveCoachInfo(coach);
          const newId =
            squad.length > 0 ? Math.max(...squad.map((x) => x.id)) + 1 : 1;
          const newPlayer = {
            ...player,
            id: newId,
            status: "reserva",
          ...
          import("../core/appCore.js").then((m) => m.saveToLocal());
          showCustomModal(
            `<strong>OFERTA ACEITA!</strong><br><br>${player.name} já está integrado ao seu elenco.`,
            "alert",
            "btn-primary",
          );
          ...
          showCustomModal(
            `<strong>PROPOSTA REJEITADA!</strong><br><br>O <strong>${player.clubName || "clube"}</strong> considerou a oferta muito baixa para liberar o atleta.`,
            "alert",
            "btn-danger",
          );
        }
...
export async function renderTraining() {
  const { getEvolutionPotential, runWeeklyTraining } =
    await import("../core/training.js");
  const coach = await Storage.getCoachInfo();
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const player = slot.playerId
      ? squad.find((p) => p.id === parseInt(slot.playerId, 10))
      : null;
    ...
    if (!player) {
      ...
      squad
        ...
        .forEach((p) => {
          // Verifica se já está selecionado em outro slot
          const isSelectedElsewhere = trainingSlots.some(
            (s, idx) => idx !== i && s.playerId === p.id.toString(),
          );
      ...
      const potentialColor =
        potential === "Jovem Promessa"
          ? "#f59e0b"
          : potential === "Auge Técnico"
            ? "#00ff88"
            : potential === "Experiente"
              ? "#00aaff"
              : "#888";

      const maxMatches = Math.max(...squad.map((p) => p.matchesPlayed || 0), 1);
      const rhythm = Math.round(
        ((player.matchesPlayed || 0) / maxMatches) * 100,
      );
      ...
      slotEl.innerHTML = `
        ...
        <div style="text-align: center; display: flex; flex-direction: column; gap: 4px; margin: 10px 0;">
          ...
          <span style="font-size: 0.75rem; color: #aaa; font-weight: bold; text-transform: uppercase;">
            ${player.aptitude?.[0] || "?"} • OVR ${Math.round(player.rating || player.ovr)} • ${player.age} anos
          </span>
        </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 8px; background: rgba(255,255,255,0.01); padding: 10px; border-radius: 10px; border: 1px solid #222; margin-bottom: auto;">
          ...
          <div style="display: flex; justify-content: space-between; font-size: 0.75rem;">
            <span style="color: #666;">Nota Média:</span>
        ...
        <div style="display: flex; flex-direction: column; gap: 5px; text-align: left;">
          ...
          <select data-slot="${i}" class="slot-focus-select" style="width: 100%; padding: 8px 10px; background: #1a1a1a; border: 1px solid #333; color: #fff; border-radius: 8px; font-size: 0.85rem; outline: none; cursor: pointer;" ${trainingUsed ? "disabled" : ""}>
            <option value="Fisico" ${slot.focus === "Fisico" ? "selected" : ""}>🏃‍♂️ Físico (Sta, Fis, Vel)</option>
            ${
              isGK
                ? `
              <option value="Goleiro" ${slot.focus === "Goleiro" ? "selected" : ""}>🧤 Goleiro (Ref, Pos, Alc)</option>
            `
                : `
              <option value="Ataque" ${slot.focus === "Ataque" ? "selected" : ""}>⚡ Ataque (Vel, Fin, Dri)</option>
              <option value="Meio" ${slot.focus === "Meio" ? "selected" : ""}>🧠 Meio-Campo (Pas, Dri, Fis)</option>
              <option value="Defesa" ${slot.focus === "Defesa" ? "selected" : ""}>🛡️ Defesa (Def, Fis, Sta)</option>
            `
            }
          </select>
        </div>
  ...
  if (tbody) {
    ...
    sortedSquad.forEach((player) => {
      ...
      const slotIndex = trainingSlots.findIndex(
        (s) => s.playerId === player.id.toString(),
      );
    ...
    document.querySelectorAll(".quick-train-btn").forEach((el) => {
      el.addEventListener("click", (e) => {
        ...
        if (firstEmptySlot !== -1) {
          ...
          showCustomModal(
            "Todos os 3 slots de treinamento já estão ocupados!",
            "alert",
            "btn-secondary",
          );
        }
  ...
  if (runButton && !trainingUsed) {
    runButton.onclick = async () => {
      ...
      if (selectedSlots.length === 0) {
        showCustomModal(
          "Por favor, selecione pelo menos um jogador para treinar!",
          "alert",
          "btn-secondary",
        );
        return;
      ...
      const proceed = await showCustomModal(
        `Deseja iniciar a sessão semanal de treinos para os <strong>${selectedSlots.length}</strong> atletas escalados?<br><br><small style="color: #aaa;">Isso consumirá a sessão desta semana.</small>`,
        "confirm",
        "btn-primary",
      );

      if (proceed) {
        dbgToast("⚡ Simulando treinamentos...", "#f59e0b");
        ...
        let reportHtml = `
          ...
          <h2 style="color: #fff; margin-bottom: 20px; font-weight: 900; letter-spacing: -0.5px;">BOLETIM DE <span style="color: #f59e0b;">EVOLUÇÃO</span></h2>
        `;

        if (logs.length === 0) {
          reportHtml += `
            <p style="color: #bbb; line-height: 1.6; font-size: 0.95rem; margin-bottom: 20px;">
            A sessão de treinos foi concluída de forma regular. Os atletas trabalharam duro, mas nenhum obteve aumento de atributos significativo nesta rodada.<br><br>
            <span style="color: #666;">Continue mantendo-os ativos em partidas e sob rotina constante de treinos para aumentar as chances de evolução!</span>
            </p>
          `;
        } else {
          reportHtml += `
          ...
          logs.forEach((log) => {
            ...
            const upgradedStatStrings = log.upgrades
              .map(
                (u) => `
              ...
              <span style="font-weight: 800; color: #00ff88;">${u.oldVal} ➔ ${u.newVal} <span style="font-size: 0.7rem; margin-left: 2px;">▲</span></span>
              </div>
            `,
              )
              .join("");
        ...
        reportHtml += `
          <button id="modalBackBtn" class="btn-primary" style="width: 100%; padding: 12px; border-radius: 8px; font-weight: bold; background: #f59e0b; color: #000; border: none;">CONCLUIR SESSÃO</button>
          </div>
        `;

        await showCustomModal(reportHtml, "custom");

        // Limpar slots locais para a próxima vez
...
export async function renderFinances() {
  ...
  const statCard = (label, value, color, icon) => `
    <div style="background:#111; border:1px solid ${color}33; border-radius:14px; padding:22px 25px; display:flex; flex-direction:column; gap:8px; position:relative; overflow:hidden;">
      <div style="position:absolute; top:15px; right:18px; opacity:0.1; color:${color};"><i data-lucide="${icon}" style="width: 3.5rem; height: 3.5rem;"></i></div>
      <div style="font-size:0.75rem; color:#777; text-transform:uppercase; letter-spacing:1.5px; font-weight:800;">${label}</div>
      <div style="font-size:2rem; font-weight:900; color:${color}; letter-spacing:-0.5px;">${value}</div>
    </div>
  `;
  ...
  const txRow = (t) => {
    ...
    const sign = isRev ? "+" : "-";
    return `
      <div style="display:flex; align-items:center; justify-content:space-between; padding:14px 18px; background:#0d0d0d; border-radius:10px; border:1px solid #1a1a1a; transition: background 0.2s;" onmouseover="this.style.background='#151515'" onmouseout="this.style.background='#0d0d0d'">
        <div style="display:flex; align-items:center; gap:14px;">
          <div style="width:10px; height:10px; border-radius:50%; background:${color}; flex-shrink:0;"></div>
          <div>
            <div style="font-size:0.95rem; font-weight:700; color:#fff; letter-spacing:0.3px;">${t.desc || "Transação"}</div>
            <div style="font-size:0.75rem; color:#666; margin-top:4px; font-weight:500;">${t.date || ""}</div>
          </div>
        </div>
        <div style="font-size:1.1rem; font-weight:900; color:${color}; letter-spacing:0.5px;">${sign}${formatVal(t.amount || 0)}</div>
      </div>
    `;
  };
  ...
  const txList =
    transactions.length > 0
      ...
      : `<div style="text-align:center; padding:40px; color:#666; font-size:0.95rem; font-weight:500; letter-spacing:0.5px;">Ainda não há registros financeiros contabilizados nesta temporada.</div>`;
    ...
    <div style="max-width:1100px; margin:0 auto; padding:25px; display:flex; flex-direction:column; gap:24px;">
      ...
      <div style="display:flex; align-items:center; justify-content:space-between; background:#111; padding:22px 28px; border-radius:16px; border:1px solid var(--border);">
        <div style="display:flex; align-items:center; gap:16px;">
          <div style="width:56px; height:56px; background:rgba(34,211,238,0.1); border-radius:50%; display:flex; align-items:center; justify-content:center; border:1px solid rgba(34,211,238,0.25);">
            <i data-lucide="landmark" style="width:30px; height:30px; color:#22d3ee;"></i>
          </div>
          <div>
            <h1 style="margin:0; font-size:2rem; font-weight:900; color:#fff; letter-spacing:-0.5px;">Departamento Financeiro</h1>
            <div style="font-size:0.85rem; color:#666; margin-top:4px; text-transform:uppercase; letter-spacing:1.5px; font-weight:700;">Balanço Anual • ${coachInfo.teamName || ""}</div>
          </div>
        </div>
        <button onclick="window.switchMainView('dashboard')" style="background:#1a1a1a; border:1px solid #333; color:#888; padding:12px 24px; border-radius:10px; cursor:pointer; font-size:0.85rem; font-weight:800; display:flex; align-items:center; gap:8px; transition:all 0.2s;" onmouseover="this.style.color='#fff'; this.style.borderColor='#555'; this.style.background='#222'" onmouseout="this.style.color='#888'; this.style.borderColor='#333'; this.style.background='#1a1a1a'">
          <i data-lucide="arrow-left" style="width: 1.1rem; height: 1.1rem;"></i> VOLTAR
        </button>
      </div>

      <!-- KPI Cards -->
      <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:16px;">
        ${statCard("Orçamento Atual", formatVal(budget), "#22d3ee", "wallet")}
        ${statCard("Receitas (Entradas)", formatVal(totalRevenue), "var(--accent)", "trending-up")}
        ${statCard("Despesas (Saídas)", formatVal(totalExpense), "#ff4444", "trending-down")}
        ${statCard("Resultado Operacional", (balance >= 0 ? "+" : "") + formatVal(balance), balanceColor, "activity")}
      </div>

      <!-- Gráfico de Barra Simples (Receita vs Despesa) -->
      ${
        totalRevenue + totalExpense > 0
          ? `
      <div style="background:#111; border:1px solid var(--border); border-radius:14px; padding:22px 25px;">
        <h3 style="margin:0 0 20px 0; font-size:0.8rem; color:#777; text-transform:uppercase; letter-spacing:1.5px; font-weight:800;">Demonstrativo Gráfico</h3>
        <div style="display:flex; flex-direction:column; gap:12px;">
          <div style="display:flex; align-items:center; gap:14px;">
            <div style="font-size:0.85rem; color:var(--accent); width:85px; text-align:right; font-weight:800; letter-spacing:0.5px;">ENTRADAS</div>
            <div style="flex:1; background:#1a1a1a; border-radius:8px; height:32px; overflow:hidden;">
              <div style="height:100%; width:${totalRevenue + totalExpense > 0 ? Math.round((totalRevenue / (totalRevenue + totalExpense)) * 100) : 0}%; background:var(--accent); border-radius:8px; transition:width 0.8s ease; display:flex; align-items:center; padding-left:12px;">
                <span style="font-size:0.8rem; font-weight:900; color:#000;">${formatVal(totalRevenue)}</span>
              </div>
            </div>
          </div>
          <div style="display:flex; align-items:center; gap:14px;">
            <div style="font-size:0.85rem; color:#ff4444; width:85px; text-align:right; font-weight:800; letter-spacing:0.5px;">SAÍDAS</div>
            <div style="flex:1; background:#1a1a1a; border-radius:8px; height:32px; overflow:hidden;">
              <div style="height:100%; width:${totalRevenue + totalExpense > 0 ? Math.round((totalExpense / (totalRevenue + totalExpense)) * 100) : 0}%; background:#ff4444; border-radius:8px; transition:width 0.8s ease; display:flex; align-items:center; padding-left:12px;">
                <span style="font-size:0.8rem; font-weight:900; color:#fff;">${formatVal(totalExpense)}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style="background:#111; border:1px solid var(--border); border-radius:14px; padding:22px 25px; flex:1;">
        <h3 style="margin:0 0 18px 0; font-size:0.8rem; color:#777; text-transform:uppercase; letter-spacing:1.5px; font-weight:800;">Extrato de Transações</h3>
        <div style="display:flex; flex-direction:column; gap:10px; max-height:380px; overflow-y:auto; padding-right:6px;">
          ${txList}
        </div>
      </div>
    </div>
